...
export const wmsInspection = (map, url, layerId, layer) => {
  ...
  map.on("click", async (e) => {
    ...
    try {
      ...
      // CUL fix: wrap overlayLayer in its own try/catch so that a Leaflet
      // rendering crash (Bounds.intersects TypeError during zoom animation)
      // does not prevent the attribute table from populating.
      // GBL5 upstream bug confirmed unfixed in GBL6 pre-alpha.
      try {
        if (!data.isHTML) overlayLayer(map, data, layer);
      } catch (geometryError) {
        console.warn("Could not overlay inspection geometry:", geometryError);
      }
